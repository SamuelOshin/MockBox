...
from app.schemas.ai_schemas import (
    MockGenerationRequest, MockGenerationResponse, AIProvider, 
    AIUsageStats, AIErrorResponse, ResponseFormat
)
...
class AIService:
    ...
    async def generate_mock_data(
        ...
        try:
            ...
            # Log usage
            await self._log_usage(user_id, provider, response_data.get("tokens_used", 0))
    ...
    async def _generate_with_openai(self, request: MockGenerationRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 429:
                ...
                raise AIProviderError(f"OpenAI API error: {response.status_code}", "openai")
            ...
            return {
                ...
                "explanation": generated_data.get("explanation", "Generated with OpenAI"),
                "model": "gpt-4o-mini",
    ...
    async def _generate_with_anthropic(self, request: MockGenerationRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 429:
                raise AIProviderError("Rate limit exceeded", "anthropic", retry_after=60)
            elif response.status_code != 200:
                raise AIProviderError(f"Anthropic API error: {response.status_code}", "anthropic")
    ...
    async def _generate_fallback(self, request: MockGenerationRequest) -> Dict[str, Any]:
...
"""
    ...
    def _generate_deterministic_data(self, request: MockGenerationRequest) -> Dict[str, Any]:
    ...
    def _parse_claude_response(self, content: str, request: MockGenerationRequest) -> Dict[str, Any]:
    ...
    async def _log_usage(self, user_id: UUID, provider: AIProvider, tokens_used: int) -> None:
        """Log AI usage for billing and monitoring"""
        logger.info(f"AI usage - User: {user_id}, Provider: {provider}, Tokens: {tokens_used}")
