...
class RateLimitMonitor:
    ...
    async def get_rate_limit_stats(self, timeframe: str = "24h") -> Dict[str, Any]:
        ...
        try:
            ...
            recent_violations = [
                v for v in self.violations_cache if v["timestamp"] > cutoff_time
            ]
            ...
            for violation in recent_violations:
                ...
                stats["violations_by_type"][v_type] = (
                    stats["violations_by_type"].get(v_type, 0) + 1
                )
                stats["violations_by_endpoint"][endpoint] = (
                    stats["violations_by_endpoint"].get(endpoint, 0) + 1
                )

                if ip:
                    stats["violations_by_ip"][ip] = (
                        stats["violations_by_ip"].get(ip, 0) + 1
                    )

                if user_id:
                    stats["violations_by_user"][user_id] = (
                        stats["violations_by_user"].get(user_id, 0) + 1
                    )

                stats["hourly_breakdown"][hour] = (
                    stats["hourly_breakdown"].get(hour, 0) + 1
                )
    ...
    async def get_suspicious_activity(
        ...
        try:
            ...
            return sorted(
                suspicious_ips, key=lambda x: x["violation_count"], reverse=True
            )
    ...
    async def cleanup_old_violations(self):
        ...
        try:
            ...
            logger.debug(
                f"Cleaned up old violations, {len(self.violations_cache)} records remaining"
            )
        except Exception as e:
...
class SecurityMonitor:
    ...
    async def log_security_event(
        ...
        if severity == "critical":
            logger.critical(
                f"Security event: {event_type} - {description}", extra=event
            )
        elif severity == "high":
