...
class MockService:
    ...
    async def create_mock(self, user_id: UUID, mock_data: MockCreate) -> Mock:
        ...
        try:
            ...
            existing = await self._get_mock_by_endpoint(user_id, mock_data.endpoint, mock_data.method)
            if existing:
                raise HTTPException(
    ...
    async def update_mock(self, mock_id: UUID, user_id: UUID, update_data: MockUpdate) -> Mock:
        """Update mock"""
        try:
    ...
    async def simulate_mock(self, mock_id: UUID, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            await self._log_mock_access(mock_id, mock.user_id, request_data, execution_time, mock.status_code)
    ...
    async def _log_mock_access(self, mock_id: UUID, user_id: UUID, request_data: Dict[str, Any], response_time: float, status_code: int):
