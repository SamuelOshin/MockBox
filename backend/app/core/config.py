...
class Settings(BaseSettings):
    ...
    public_api_rate_limit: int = Field(
        default=100, env="PUBLIC_API_RATE_LIMIT"
    )  # per minute
    authenticated_rate_limit: int = Field(
        default=1000, env="AUTHENTICATED_RATE_LIMIT"
    )  # per minute
    anonymous_rate_limit: int = Field(
        default=60, env="ANONYMOUS_RATE_LIMIT"
    )  # per minute
    simulation_rate_limit: int = Field(
        default=200, env="SIMULATION_RATE_LIMIT"
    )  # per minute
    # Security settings
    enable_security_headers: bool = Field(default=True, env="ENABLE_SECURITY_HEADERS")
    enable_authentication_middleware: bool = Field(
        default=True, env="ENABLE_AUTHENTICATION_MIDDLEWARE"
    )
    enable_security_validation: bool = Field(
        default=True, env="ENABLE_SECURITY_VALIDATION"
    )
    suspicious_activity_threshold: int = Field(
        default=50, env="SUSPICIOUS_ACTIVITY_THRESHOLD"
    )  # violations per hour
