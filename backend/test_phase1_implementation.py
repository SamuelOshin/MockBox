...
def test_configuration():
    ...
    try:
        ...
        ), "enable_rate_limiting setting missing"
        assert hasattr(
            settings, "enable_security_headers"
        ), "enable_security_headers setting missing"
        assert hasattr(
            settings, "enable_authentication_middleware"
        ), "enable_authentication_middleware setting missing"
        assert hasattr(
            settings, "enable_security_validation"
        ), "enable_security_validation setting missing"

        # Test rate limiting settings
        assert hasattr(settings, "ai_rate_limit"), "ai_rate_limit setting missing"
        assert hasattr(
            settings, "public_api_rate_limit"
        ), "public_api_rate_limit setting missing"
        assert hasattr(
            settings, "authenticated_rate_limit"
        ), "authenticated_rate_limit setting missing"
        assert hasattr(
            settings, "anonymous_rate_limit"
        ), "anonymous_rate_limit setting missing"
        assert hasattr(
            settings, "simulation_rate_limit"
        ), "simulation_rate_limit setting missing"
        ...
        print(
            f"   Auth middleware enabled: {settings.enable_authentication_middleware}"
        )
...
def test_rate_limiter():
    ...
    try:
        ...
        assert hasattr(
            limiter, "get_rate_limit_key"
        ), "get_rate_limit_key method missing"
...
def test_middleware_classes():
    ...
    try:
        ...
        ), "RateLimitMiddleware missing __init__"
        assert hasattr(
            SecurityHeadersMiddleware, "__init__"
        ), "SecurityHeadersMiddleware missing __init__"
        assert hasattr(
            AuthenticationMiddleware, "__init__"
        ), "AuthenticationMiddleware missing __init__"
        assert hasattr(
            SecurityValidationMiddleware, "__init__"
        ), "SecurityValidationMiddleware missing __init__"
